...
def get_ball_tree(filename_ball_tree):
    ...
    ball_tree_filename = Path('data/ball_trees') / filename_ball_tree
    if ball_tree_filename.exists():
        tile_ball_tree = joblib.load(ball_tree_filename)
        return tile_ball_tree
    else:
        raise FileNotFoundError(f"No coastal data found for tile {filename_ball_tree}")
...
def main(lat, lon):
    ...
    if filename_h5:
        ...
        filename_ball_tree = filename_h5.replace('.h5', '_coastal_points_ball_tree.joblib')
        try:
            ...
            print("Pathway 1: Point is within a tile. but there are no points near coast within this tile ")
            print("ball tree does not exist, tile is all land")
            nearest_point, distance_m = coord_to_coastal_point(lat, lon)
            filename_h5 = get_filename_for_coordinates(nearest_point[0], nearest_point[1], bounds_dict)
            filename_ball_tree = filename_h5.replace('.h5', '_coastal_points_ball_tree.joblib')
            tile_ball_tree = get_ball_tree(filename_ball_tree)
            distance_m, nearest_point = ball_tree_distance(tile_ball_tree, [lat, lon])
            return distance_m, land_class, nearest_point
...
if __name__ == "__main__":
    ...
    print(f"Result: {distance_m} meters to coast, land cover class: {land_water_mapping[land_or_water]}, nearest coastal point: {nearest_point}")
